...
package sourcerepresentationinstance
...
// Setup adds a controller that reconciles SourceRepresentationInstance managed resources.
func Setup(mgr ctrl.Manager, o tjcontroller.Options) error {
	name := managed.ControllerName(v1beta1.SourceRepresentationInstance_GroupVersionKind.String())
	...
	// register webhooks for the kind v1beta1.SourceRepresentationInstance
	// if they're enabled.
	if o.StartWebhooks {
		if err := ctrl.NewWebhookManagedBy(mgr).
			For(&v1beta1.SourceRepresentationInstance{}).
			Complete(); err != nil {
			return errors.Wrap(err, "cannot register webhook for the kind v1beta1.SourceRepresentationInstance")
		}
	}

	r := managed.NewReconciler(mgr, xpresource.ManagedKind(v1beta1.SourceRepresentationInstance_GroupVersionKind), opts...)

	return ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o.ForControllerRuntime()).
		WithEventFilter(xpresource.DesiredStateChanged()).
		Watches(&v1beta1.SourceRepresentationInstance{}, eventHandler).
